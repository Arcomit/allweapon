...
public class BaiYueDingHandler {
    private static final StructurePattern STRUCTURE_PATTERN = new StructurePattern()
            .addBlock(new BlockPos(0,-1,0), Blocks.SNOW_BLOCK)
            .addBlock(new BlockPos(0,-1,1), Blocks.OBSIDIAN)
            .addBlock(new BlockPos(0,-1,-1), Blocks.OBSIDIAN)
            .addBlock(new BlockPos(1,-1,0), Blocks.OBSIDIAN)
            .addBlock(new BlockPos(-1,-1,0), Blocks.OBSIDIAN);
    ...
    public static void bladeStandOnHit(SlashBladeEvent.BladeStandAttackEvent event) {
        ...
        if (!slashBladeDefinitionRegistry.containsKey(AwBlades.BAIYUEDING)) {
            return;
        }
        ...
        var in = SlashBladeIngredient.of(RequestDefinition.Builder.newInstance()
                .refineCount(100)
                .addEnchantment(
                        new EnchantmentDefinition(EnchantmentUtils.getEnchantmentID(Enchantments.ALL_DAMAGE_PROTECTION), 2)
                )
                .build()
        );
        if (!in.test(blade)){
        ...
        if (STRUCTURE_PATTERN.check(level, standPos) != null) {
            ItemStack newBlade = Objects.requireNonNull(slashBladeDefinitionRegistry.get(AwBlades.BAIYUEDING)).getBlade();
