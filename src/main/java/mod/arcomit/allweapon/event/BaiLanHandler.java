...
public class BaiLanHandler {
    ...
    public static void bladeStandOnHit(SlashBladeEvent.BladeStandAttackEvent event) {
        ...
        if (!slashBladeDefinitionRegistry.containsKey(AwBlades.BAILAN)) {
            return;
        }
        ...
        if (STRUCTURE_PATTERN.check(level, standPos) != null) {
            ItemStack newBlade = Objects.requireNonNull(slashBladeDefinitionRegistry.get(AwBlades.BAILAN)).getBlade();
            ...
            level.setBlock(standPos, Blocks.AIR.defaultBlockState(), 3);
