...
public class BaiQiYueHandler {
    ...
    public static void bladeStandOnHit(SlashBladeEvent.BladeStandAttackEvent event) {
        ...
        if (!slashBladeDefinitionRegistry.containsKey(AwBlades.BAIQIYUE)) {
            return;
        }
        ...
        if (STRUCTURE_PATTERN.check(level, standPos) != null) {
            ItemStack newBlade = Objects.requireNonNull(slashBladeDefinitionRegistry.get(AwBlades.BAIQIYUE)).getBlade();
